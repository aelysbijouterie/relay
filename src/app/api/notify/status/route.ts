...
export async function POST(request: NextRequest) {
  const { assignees, task, oldStatus, department, changedByName } = await request.json()

  if (!assignees?.length) return NextResponse.json({ sent: 0 })

  const results = await Promise.allSettled(
    assignees.map(async (a: { name: string; email: string }) => {
      const tpl = emailStatusChange({ assigneeName: a.name, changedByName, task, oldStatus, department })
